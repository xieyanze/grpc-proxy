 ...
 *
 * <p>A change in stream state is propagated through the channel pipeline as a user event via {@link
 * Http2StreamStateEvent} objects. When a HTTP/2 stream first becomes active a {@link Http2StreamActiveEvent} and when
 * it gets closed a {@link Http2StreamClosedEvent} is emitted.
 *
 ...
 *
 * <p><em>This API is very immature.</em> The Http2Connection-based API is currently preferred over this API. This API
 * is targeted to eventually replace or reduce the need for the Http2Connection-based API.
 */
@UnstableApi
